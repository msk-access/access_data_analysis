...
compile_reads_all <- function(master.ref,
                          results.dir,
                          project.ID,
                          pooled.bam.dir = "/data1/core006/access/production/resources/msk-access/v2.0/novaseq_curated_duplex_bams_dmp/current/",
                          fasta.path = "/data1/core006/access/production/resources/reference/versions/hg19_virus_special/hg19_virus.fasta",
                          genotyper.path = "/data1/core006/access/production/resources/tools/GetBaseCountsMultiSample/current/GetBaseCountsMultiSample",
                          dmp.dir = "/data1/core006/access/production/resources/cbioportal/versions/msk-impact/msk_solid_heme/",
  ...
  # # test input section -----------------------------------------------------------
  # master.ref = fread('/juno/data1/core006/bergerm1/bergerlab/zhengy1/access_data_analysis/data/example_master_file.csv')
  # results.dir = paste0('/juno/data1/core006/bergerm1/MSK-ACCESS/ACCESS-Projects/test_access/access_data_analysis/output_',format(Sys.time(),'%m%d%y'))
  # pooled.bam.dir = '/ifs/data1/core006/bergerm1/ACCESS-Projects/novaseq_curated_duplex_v2/'
  # fasta.path = '/data1/core006/access/production/resources/reference/current/Homo_sapiens_assembly19.fasta'
  # genotyper.path = '/ifs/data1/core006/bergerm1/Innovation/software/maysun/GetBaseCountsMultiSample/GetBaseCountsMultiSample'
  # dmp.dir = '/ifs/data1/core006/bergerm1/zhengy1/dmp/mskimpact/'
    ...
    lapply(unique(master.ref$cmo_patient_id), function(x) {
      ...
      system(
		paste0(
  		...
  		' --mem=4G',                                     # Memory (4GB)
  		' --chdir=', results.dir, "/", x, "/",         # Working directory
  		' --output=hotspot.o',                          # Standard output
  		' --error=hotspot.e',                           # Standard error
  		' --time=00:59:00',                             # Time limit (59 minutes)
  		' --job-name=tag_hotspot',             # Job name
  		' --wrap="python /data1/core006/access/production/workflows/access_workflows/v1/pipeline_2.0.0/ACCESS-Pipeline/cwl_tools/hotspots/tag_hotspots.py',
  		' -m ', results.dir, "/", x, "/", x, '_all_unique_calls.maf',
  		' -itxt /data1/core006/access/production/resources/msk-access/v2.0/regions_of_interest/versions/v1.0/hotspot-list-union-v1-v2_with_TERT.txt',
      ...
      job.ids <- system(
		paste0(
			...
			" --partition=cmobic_cpu",
  			...
  			" --output=genotyping.o",
  			" --error=genotyping.e",
  			" --job-name=genotype_variants",
  			' --wrap="genotype_variants small_variants multiple-samples',
  			" -i ", results.dir, "/", x, "/", x, "_genotype_metadata.tsv",
  			" -r ", fasta.path,
  			" -g ", genotyper.path,
  			' -v DEBUG"'
		),
  ...
  pooled.sample.job.id <- system(
    paste0(
		"sbatch",
		" --partition=cmobic_cpu",
  		" --chdir=", results.dir, "/pooled",
  		" --time=12:00:00",
  		" --mem=8G",
  		" --output=genotyping.o",
  		" --error=genotyping.e",
  		paste0(unlist(all.fillout.id), collapse = ":"),
  		" --job-name=pooled_genotype_variants",
  		' --wrap="genotype_variants small_variants multiple-samples',
  		" -i ", results.dir, "/pooled/pooled_metadata.tsv",
  		" -r ", fasta.path,
  		" -g ", genotyper.path,
  		' -v DEBUG"'
    ),
  ...
  while (length(system(paste0("squeue -j ", pooled.sample.job.id, " -h"), intern = TRUE)) > 0) {
  Sys.sleep(120)
	}
...
if (!interactive()) {
  ...
  parser$add_argument(
    ...
    "--pooledbamdir",
    type = "character",
    default = "/data1/core006/access/production/resources/msk-access/v2.0/novaseq_curated_duplex_bams_dmp/current/",
  ...
  parser$add_argument(
    ...
    "--fastapath",
    type = "character",
    default = "/data1/core006/access/production/resources/reference/versions/hg19_virus_special/hg19_virus.fasta",
  ...
  parser$add_argument(
    ...
    "--genotyperpath",
    type = "character",
    default = "/data1/core006/access/production/resources/tools/GetBaseCountsMultiSample/versions/GetBaseCountsMultiSample-1.2.5/GetBaseCountsMultiSample",
  ...
  parser$add_argument(
    ...
    "--dmpdir",
    type = "character",
    default = "/data1/core006/access/production/resources/cbioportal/versions/msk-impact/msk_solid_heme/",
  ...
  parser$add_argument(
    ...
    "--mirrorbamdir",
    type = "character",
    default = "/data1/share001/share/impact_12_245/",
  ...
  parser$add_argument(
    ...
    "--mirroraccessbamdir",
    type = "character",
    default = "/data1/share001/share/access_12_245/",
  ...
  parser$add_argument(
    ...
    "--dmpkeypath",
    type = "character",
    default = "/data1/share001/share/impact_12_245.key.txt",
  ...
  parser$add_argument(
    ...
    "--dmpaccesskeypath",
    type = "character",
    default = "/data1/share001/share/access_12_245.key.txt",
