  ...
  all.fillout.id = lapply(unique(master.ref$cmo_patient_id),function(x){
    ...
    if(!is.na(dmp_id)){
      ...
      dmp.sample.sheet = data.frame(Sample_Barcode = all.dmp.ids,
                                    standard_bam = paste0(mirror.bam.dir,'/',bam.sub.dir,'/',all.dmp.bam.ids) %>%
    ...
    all.calls = rbind(duplex.calls[,intersect(colnames(duplex.calls),colnames(DMP.RET.maf)),with = F],
    ...
    write.table(sample.sheet[,.(sample_id = Sample_Barcode,maf = paste0(results.dir,'/',x,'/',x,'_all_unique_calls.maf'),
                ...
                paste0(results.dir,'/',x,'/',x,'_genotype_metadata.tsv'),sep = '\t',quote = F,row.names = F)
...
if (!interactive()) {
  ...
  parser$add_argument('-pid', '--projectid', type='character',default = '', 
