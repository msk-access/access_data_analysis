...
tab <- fread(args$input) %>%
  mutate(
    ...
    endtouse = as.Date(endtouse, format = "%m/%d/%y"),
    ...
    end_numeric = time_diff(collection_date, endtouse, args$timeunit)
  ) %>%
  mutate(within100days = factor(ifelse(collection_date_numeric >= -100 & end_numeric <= 100, "yes", "no"), levels = c("yes", "no"))) %>%
...
plot0 <- tab %>% ggplot(aes(y = fct_reorder(patient_id, endtouse_numeric))) +
  ...
  scale_fill_brewer(name = "", palette = "Accent") + xlab("# of Samples")

# Plot 1
plot1 <- tab %>% filter(collection_date_numeric >= -150 & end_numeric <= 500) %>% 
  ...
  theme(legend.title = element_blank(), axis.title.y = element_blank()) + xlab("Days from Treatment Start") + 
  scale_shape_manual(values = c(21, 8)) + guides(shape = "none", color = "none", fill = "none") + 
  scale_fill_manual(values = c("old" = NA, "new" = "#FFFF99", "reseq" = "transparent"), na.value = "transparent", name = "Sample Type") +
...
# Plot 2
plot2 <- tab %>% filter(collection_date_numeric >= -150 & end_numeric <= 500) %>% 
