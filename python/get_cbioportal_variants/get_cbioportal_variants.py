...
def subset_cna(
    ...
    subset_tsv = filter_by_column(ids_to_subset,cna_df)
...
def subset_sv(
    ...
    sv_df = read_tsv(sv)
    ids_to_subset = read_ids(sid,ids)
    subset_tsv = filter_by_row(ids_to_subset,sv_df,col_name)
    subset_tsv.drop_duplicates().to_csv(output_file, sep="\t", index=False)
...
def subset_maf(
    ...
    maf_df = read_tsv(maf)
    ids_to_subset = read_ids(sid,ids)
    subset_maf = filter_by_row(ids_to_subset,maf_df,col_name)
...
def filter_by_row(sid,tsv_df):
    # filter for ids
    ns = sid.insert(0,"Hugo_Symbol")
    subset_tsv = tsv_df.filter(items=ns)
    return subset_tsv.copy(deep=True)

def filter_by_column(sid,tsv_df,col_name):
    # filter for ids
    ns = set(sid)
...
def check_if_covered(bedObj, mafObj):
    ...
    mafObj["covered"] = bedObj.lookup_df(
        mafObj, "Chromosome", "Start_Position"
    )
