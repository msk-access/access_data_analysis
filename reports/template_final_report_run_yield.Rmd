...
output_dir<-"{{OUTPUT_DIR}}" #AH

## read metadata files ##
metadata <- fread(metadata_file)

#AH - read variant table
var_qc <- read.csv(paste0(output_dir, "/", sample, "_variant_table.csv"), header = TRUE)
...
if ("timepoint" %in% colnames(metadata)) {
  ...
  metadata <- metadata[,c("cmo_sample_id", "sex", "collection_day", "timepoint")]
  ...
  metadata <- metadata[,c("cmo_sample_id", "sex", "collection_day")]
}

#added AH
names(metadata)[c(1,3)] <- c("sample_id", "collection_day")
```
...
#AH remove variants that failed QC
vars.keep <- var_qc[var_qc$QC == "PASS",]
vars.keep$var_id <- paste0(vars.keep$Hugo_Symbol, vars.keep$Start_Position, vars.keep$Tumor_Seq_Allele2)
keep.idx <- ifelse(paste0(final$Hugo_Symbol, final$Start_Position, final$Tumor_Seq_Allele2) %in%
    vars.keep$var_id, TRUE, FALSE)
final <- final[keep.idx,]
...
#AH - change x-labels to collection day
xlabels <- final$collection_day
```
...
unique(final[, cols][order(final$collection_day),]) %>% kable() %>% kable_styling()
```
...
```{r}
yield_data <- fread(metadata_file)
yield_data <- yield_data[,c("cmo_sample_id", "collection_day", "ctdna_vol", "ctdna_conc", "ctdna_yield")]
names(yield_data)[c(3,4,5)] <- c("Volume.Extracted_mL", "cfDNA_Conc_ng_uL",
  "cfDNA_Total_Yield_ng")

#vaf_data <- final
#print(vaf_data)

#vaf.grid <- ggplot(data = vaf_data, aes(x = collection_day, y = VAF, group = cmoPatientId)) + geom_line(color = "darkslategray", size = 0.75) + geom_point(color = "darkslategray", size = 1.2) + geom_point(data = subset(vaf_data, VAF == 0), color = "white", size = 0.5) + theme_minimal() + theme(legend.position = "none", panel.background = element_rect(fill = "aliceblue", colour = "aliceblue"), axis.title.x = element_blank(), axis.text.x = element_blank()) + ylab("VAF") + labs(title = patient) + coord_cartesian(ylim = c(0, NA), xlim = c(-20,200))
  
yield_plot <- ggplot(data = yield_data, aes(x = collection_day, y = cfDNA_Total_Yield_ng)) + geom_point() + ylab("Total Yield (ng)") + xlab("Treatment Day") + ggtitle("cfDNA Total Yield") + scale_y_continuous(labels = number_format(accuracy = 0.1)) + coord_cartesian(ylim = c(0, NA), xlim = c(-20,200))

volume_plot <- ggplot(data = yield_data, aes(x = collection_day, y = Volume.Extracted_mL)) + geom_point() + ylab("Volume Extracted (mL)") + xlab("Treatment Day") + ggtitle("Volume Extracted") + scale_y_continuous(labels = number_format(accuracy = 0.1)) + coord_cartesian(ylim = c(0, NA), xlim = c(-20,200))

concentration_plot <- ggplot(data = yield_data, aes(x = collection_day, y = cfDNA_Conc_ng_uL)) + geom_point() + ylab("cfDNA Concentration (ng/uL)") + xlab("Treatment Day") + ggtitle("cfDNA Concentration") + scale_y_continuous(labels = number_format(accuracy = 0.1)) + coord_cartesian(ylim = c(0, NA), xlim = c(-20,200))


# Convert ggplot objects to plotly objects
yield_plotly <- ggplotly(yield_plot)
volume_plotly <- ggplotly(volume_plot)
concentration_plotly <- ggplotly(concentration_plot)

# Use subplot to combine the plots
subplot(yield_plotly, volume_plotly, concentration_plotly, nrows = 3, shareX = TRUE, heights = c(0.40, 0.30, 0.30))

#subplot(yield_plot,volume_plot,concentration_plot, nrows=3,shareX=TRUE, heights=c(0.20,0.20,0.20))
```
