...
if (dmp_id != "") {
  ...
  cna_patters <- "{{PATIENT_ID}}|{{DMP_ID}}"
} else {
  page_title = "{{PATIENT_ID}}"
  cna_patters <- "{{PATIENT_ID}}"
}
```
...
cna_plot<-function(cna, xlimits, xbreaks, xlabels){
  ...
  fig<-ggplot(cna, aes(x=collection_day, sampleid=sample_id, gene=Hugo_Symbol)) +
    geom_bar(aes(y=1, fill=fc), stat="identity",width=5) +
...
## read CNA files ##
all.cna.files <- list.files(path = access_data_analysis_cna_dir, pattern = cna_pattern, full.names=TRUE)
