...
if ("timepoint" %in% colnames(metadata)) {
  ...
  metadata <- metadata[,c("cmo_sample_id_plasma", "sex", "collection_day", "timepoint")]
  ...
  metadata <- metadata[,c("cmo_sample_id_plasma", "sex", "collection_day")]
}
...
vaf_plot <- function(variant_table, xlimits, xbreaks, xlabels, varcolors, yaccuracy=0.01, log=FALSE, cnadjusted=FALSE){
  ...
  if(log == FALSE){
    if(cnadjusted==FALSE){
      fig<-ggplot(tmp, aes(x=collection_day, y=vaf, col=VarName, hotspot=Hotspot, sampleid=cmo_sample_id_plasma)) +
        scale_y_continuous(labels = scales::percent_format(accuracy = yaccuracy)) +
        ylab("VAF")
      } else{
        fig<-ggplot(tmp, aes(x=collection_day, y=adjustedvaf, vaf=vaf, col=VarName, hotspot=Hotspot, sampleid=cmo_sample_id_plasma)) +
          scale_y_continuous(labels = scales::percent_format(accuracy = yaccuracy)) +
          ylab("Adjusted VAF")
      }
  } else{
    if(cnadjusted==FALSE){
      ...
      fig<-ggplot(tmp, aes(x=collection_day, y=vafforlog, vaf=vaf, col=VarName, hotspot=Hotspot, sampleid=cmo_sample_id_plasma)) +
        scale_y_log10(labels = scales::percent_format(accuracy = yaccuracy)) +
        ylab("VAF")
    } else{
      tmp$vafforlog = tmp$adjustedvaf + 0.0001
      fig<-ggplot(tmp, aes(x=collection_day, y=vafforlog, adjustedvaf=adjustedvaf, vaf=vaf, col=VarName, hotspot=Hotspot, sampleid=cmo_sample_id_plasma)) +
        scale_y_log10(labels = scales::percent_format(accuracy = yaccuracy)) +
        ylab("VAF")
    }
  }
  ...
  fig<-fig + geom_point() +
    ...
    xlab("")
  if(cnadjusted==FALSE){
    fig<-ggplotly(fig, tooltip=c("collection_day","vaf","VarName","Hotspot","cmo_sample_id_plasma"))
  } else{
    fig<-ggplotly(fig, tooltip=c("collection_day","vaf","adjustedvaf","VarName","Hotspot","cmo_sample_id_plasma"))
  }
  fig<-ggplotly(fig)
  return(fig)
}
...
treatment_plot <- function(treatment_table, xbreaks, xlimits, colors, ids=c("collection_day","cmo_sample_id_plasma")){
...
cna_plot<-function(cna, xlimits, xbreaks){
  ...
  fig<-ggplot(cna, aes(x=collection_day, sampleid=cmo_sample_id_plasma, gene=Hugo_Symbol)) +
  ...
  fig<-ggplotly(fig, tooltip=c("collection_day","CNA","cmo_sample_id_plasma","fc","Hugo_Symbol"))
  return(fig)
}
...
```{r read_input1, include=FALSE, echo=TRUE, eval=show_text}
...
cmo_sample_id_order<-metadata$cmo_sample_id_plasma[order(metadata$collection_day)]
...
cols <- c("cmo_sample_id_plasma", "collection_day")
...
if (!is.null(timepointcolors) && nrow(final_snv) > 0) {
  treatment_table<-unique(final[,c("cmo_sample_id_plasma","collection_day","timepoint")])
  fig1<-treatment_plot(treatment_table, xbreaks, xlimits, timepointcolors, ids=c("collection_day","cmo_sample_id_plasma"))
...
if (nrow(clonal)>0) {
  ...
  if (length(unique(clonal$VarName))>1) {
    ...
    clonal_mean <- data.frame(
      clonal[,c("vaf", "adjustedvaf", "collection_day", "cmo_sample_id_plasma")] %>%
      group_by(cmo_sample_id_plasma, collection_day) %>%
      summarize_all(mean) %>%
      mutate(clonality="CLONAL", VarName="average")
    )

    common_cols <- c("cmo_sample_id_plasma", "collection_day", "vaf", "adjustedvaf", "Hotspot", "clonality", "VarName")
...
if (nrow(clonal) > 0) {
  print_table(clonal[,c("cmo_sample_id_plasma","collection_day","VarName","tcn","expected_alt_copies","ncn", "vaf","adjustedvaf")])
}
```
