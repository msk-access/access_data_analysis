...
vaf_plot <- function(variant_table, xlimits, xbreaks, xlabels, varcolors, yaccuracy=0.01, log=FALSE, cnadjusted=FALSE){
  ...
  fig<-fig + geom_point() +
    ...
    scale_x_discrete(breaks=xbreaks, limits=xlimits, labels=xlabels) +
    labs(y="VAF") +
    xlab("")
  if(cnadjusted==FALSE){
...
treatment_plot <- function(treatment_table, xbreaks, colors, ids=c("collection_day","sample_id")){
...
cna_plot<-function(cna, xlimits, xbreaks){
  ...
  fig<-ggplot(cna, aes(x=collection_day, sampleid=sample_id, gene=Hugo_Symbol)) +
    ...
    scale_x_discrete(breaks=xbreaks, limits=xlimits) +
...
xlimits<-c(min(final$collection_day)-50, max(final$collection_day)+50)
...
if (!is.null(timepointcolors) && nrow(final_snv) > 0) {
  ...
  fig1<-treatment_plot(treatment_table, xbreaks, timepointcolors, ids=c("collection_day","sample_id"))
