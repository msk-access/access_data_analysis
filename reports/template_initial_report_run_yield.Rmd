...
yield_plot <- ggplot(data = yield_data, aes(x = collection_day, y = cfDNA_Total_Yield_ng)) + geom_point() + ylab("Total Yield (ng)") + xlab("Treatment Day") + scale_y_continuous(labels = number_format(accuracy = 0.1)) + theme_minimal() + coord_cartesian(ylim = c(0, NA), xlim = c(-20,200))

concentration_plot <- ggplot(data = yield_data, aes(x = collection_day, y = cfDNA_Conc_ng_uL)) + geom_point() + ylab("Total Yield (ng)") + xlab("Treatment Day") + scale_y_continuous(labels = number_format(accuracy = 0.1)) + theme_minimal() + coord_cartesian(ylim = c(0, NA), xlim = c(-20,200))

print(yield_plot)
print(concentration_plot)
subplot(yield_plot,concentration_plot, nrows=2,shareX=TRUE, heights=c(0.5,0.50))
...
fig2 <- ggplot(data = yield_data, aes(x = collection_day, y = cfDNA_Total_Yield_ng)) + geom_point() + ylab("Total Yield (ng)") + xlab("Treatment Day") + scale_y_continuous(labels = number_format(accuracy = 0.1)) + theme_minimal() + coord_cartesian(ylim = c(0, NA), xlim = c(-20,200))
...
if (!is.null(timepointcolors) && nrow(final_snv) > 0) {
  ...
  fig3<-vaf_plot(final_snv, xlimits, xbreaks, xlabels, varcolors, yaccuracy=0.01, log=FALSE)
  subplot(fig1, fig2, fig3, nrows=3,shareX=TRUE, heights=c(0.1,0.15,0.75))
} else if (nrow(final_snv) > 0) {
  fig3<-vaf_plot(final_snv, xlimits, xbreaks, xlabels, varcolors, yaccuracy=0.01, log=FALSE)
  subplot(fig2,fig3, nrows=2,shareX=TRUE, heights=c(0.30,0.70))
}
...
```{r}
#AH
out.table <- subset(tabletoprint, tabletoprint$VarType=="SNV_INDEL")
out.table <- cbind(out.table, QC = "", Notes = "")
out.table.filename <- paste0(output_dir, "/", sample, "_variant_table.csv")
if (!file.exists(out.table.filename)){
  write.csv(out.table, out.table.filename, row.names=FALSE)
}
```
